...
class BatchNorm:
    def __init__(self, shape, requires_grad=True, affine=True, is_test=False, **kwargs):
        if affine:
            ...
            # self.gamma = Parameter(np.ones(shape), requires_grad, True)
            # self.beta = Parameter(np.zeros(shape), requires_grad, True)
            self.requires_grad = requires_grad
        self.eps = 1e-5
        self.affine = affine
        ...
        self.overall_ave = Parameter(np.ones(shape), False)
